            ...
            VStack(spacing: 30) {
                ...
                VStack {
                    HStack {
                        ...
                        Button {
                            viewModel.isProductViewPresented = true
                        } label: {
                            Image("buyIcon")
                                .resizable()
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    LazyVGrid(columns: [.init(), .init()]) {
                        ForEach(Array(viewModel.selectedProducts.enumerated()), id: \.element.id) { _, product in
                            ProductGridCell(product: product, imageHeight: screenHeight * 0.15, gridHeight: screenHeight * 0.2)
                        }
                    }
                }
                .padding(.horizontal, screenWidth * 0.08)
            }
        }.fullScreenCover(isPresented: $viewModel.isProductViewPresented) {
            OrderUIView()
        }
    }
}
